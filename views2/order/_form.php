...
<div class="order-form">
    ...
    <?= $form->field($model, 'status_id', ['options' => ['style' => $model->isNewRecord ? 'display:none;' : '']])->dropDownList(
        \yii\helpers\ArrayHelper::map(\app\models\Status::find()->all(), 'id', 'name'),
        ['prompt' => '', 'value' => '1','onchange' => 'checkStatus(this)']
    ) ?>

    <?php if(!$model->isNewRecord && !Yii::$app->user->isGuest && !Yii::$app->user->identity->isAdmin()): ?>
